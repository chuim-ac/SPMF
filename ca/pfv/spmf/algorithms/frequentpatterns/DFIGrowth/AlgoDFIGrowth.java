...
public class AlgoDFIGrowth {
	...
	public static void DFIgrowthReady(ArrayList<LinkNode> linknode) throws IOException {
		...
		for(int i=linknode.size()-1;i>=0;i--) {
			...
			while(Hnode!=null) {
				...
				if(sublist.size()!=0) {
					if(sublist.size()>maxitemcount) {
						maxitemcount = sublist.size();
					}
			...
			DFIgrowth(linknode.get(i).hname,maxcount,list,maxitemcount); 
		}
		MemoryUsage(); 
	 ...
	 */
	public static void DFIgrowth(String strname,int hcount,ArrayList<ArrayList<String>> list,int maxitemnum) throws IOException {
		...
		if(list.size()==0) { 
			...
			if(list.size()==1) {
				...
				if(maxitemnum==allitem.size()) {
					GenSubset(strname,hcount,allitem);
					return;
				}
				...
				for(int i=newheaderTable.size()-1;i>=0;i--) {
					...
					DFIgrowth(loopstr,maxcount,newlist,maxitemcount);
				}
				MemoryUsage(); 
				sortoutputS(strname,String.valueOf(hcount));
			}
